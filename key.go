...
type key[B KeyBits] struct {
	len     uint8
	offset  uint8
	content B
}

//func NewKey[B KeyBits](content B, offset, len uint8) key[B] {
//	ko := KeyOps[B]{}
//	content = ko.bo.BitsClearedFrom(content, len)
...
//func key6FromPrefix(p netip.Prefix) key[uint128] {
//	addr := p.Addr()
//	// TODO len could be -1
//	len := uint8(p.Bits())
