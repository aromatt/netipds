...
func TestIsBitSet(t *testing.T) {
	tests := []struct {
		...
		{uint128{0, 0}, 0, bitL},
		{uint128{0, 1}, 0, bitL},
		{uint128{1 << 63, 0}, 0, bitR},
		{uint128{1 << 62, 0}, 1, bitR},
		{uint128{0, 1 << 63}, 64, bitR},
		{uint128{0, 1}, 127, bitR},
		{uint128{0, 2}, 126, bitR},
		{uint128{^uint64(0), ^uint64(0)}, 0, bitR},
		{uint128{^uint64(0), ^uint64(0)}, 127, bitR},

		// i > 127 => false
		{uint128{0, 0}, 128, bitL},
	}
	for _, tt := range tests {
		if got := tt.a.isBitSet(tt.i); got != tt.want {
			t.Errorf("%v.isBitSet(%d) = %v, want %v", tt.a, tt.i, got, tt.want)
		}
	}
}
