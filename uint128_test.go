...
func TestIsBitZero(t *testing.T) {
	tests := []struct {
		...
		{uint128{0, 0}, 0, true},
		{uint128{0, 1}, 0, true},
		{uint128{1 << 63, 0}, 0, false},
		{uint128{1 << 62, 0}, 1, false},
		{uint128{0, 1 << 63}, 64, false},
		{uint128{0, 1}, 127, false},
		{uint128{0, 2}, 126, false},
		{uint128{^uint64(0), ^uint64(0)}, 0, false},
		{uint128{^uint64(0), ^uint64(0)}, 127, false},

		// i > 127 => true
		{uint128{0, 0}, 128, true},
	}
	for _, tt := range tests {
		if got := tt.a.isBitZero(tt.i); got != tt.want {
			t.Errorf("%v.isBitZero(%d) = %v, want %v", tt.a, tt.i, got, tt.want)
		}
	}
}
