...
func (m *PrefixMapBuilder[T]) Remove(p netip.Prefix) error {
	...
	if p.Addr().Is4() {
		m.tree4 = *m.tree4.remove(key4FromPrefix(p.Masked()))
	} else {
		m.tree6 = *m.tree6.remove(key6FromPrefix(p.Masked()))
	}
	return nil
}
...
func (s *PrefixSetBuilder) SubtractPrefix(p netip.Prefix) error {
	...
	if p.Addr().Is4() {
		s.tree4 = *s.tree4.subtractKey(key4FromPrefix(p.Masked()))
	} else {
		s.tree6 = *s.tree6.subtractKey(key6FromPrefix(p.Masked()))
	}
	return nil
}
