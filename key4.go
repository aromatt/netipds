...
func newKey4(content uint128, offset uint8, len uint8) key4 {
	return key4{content.bitsClearedFrom(len), offset, len}
}

// rooted returns a copy of key with offset set to 0
func (k key4) rooted() key4 {
	return key4{k.content, 0, k.len}
}
...
func key4FromPrefix(p netip.Prefix) key4 {
	...
	return newKey4(u128From16(addr.As16()), 0, bits)
}
...
func (k *key4) Parse(s string) error {
	var err error
...
func (k key4) truncated(n uint8) key4 {
	return newKey4(k.content, k.offset, n)
}

// rest returns a copy of k starting at position i. if i > k.len, returns the
// zero key.
func (k key4) rest(i uint8) key4 {
	if k.isZero() {
		return k
	}
	if i > k.len {
...
func (k key4) commonPrefixLen(o key4) (n uint8) {
...
func (k key4) isPrefixOf(o key4, strict bool) bool {
...
func (k key4) next(b bit) (ret key4) {
	switch b {
	case bitL:
		ret = key4{
			content: k.content,
