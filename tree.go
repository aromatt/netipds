...
func (t *tree[B]) newNode(k key[B]) nodeRef {
	//t.key = append(t.key, k)
	//func (t *tree[B]) newNode(b B, s seg) nodeRef {
...
func newTree[B KeyBits[B]](nodesCap int) *tree[B] {
	t := &tree[B]{
...
func (t *tree[B]) setValue(n nodeRef) nodeRef {
	t.entry[n] = true
	return n
}

// clearEntry removes the entry and value from t.
func (t *tree[B]) clearEntry(n nodeRef) {
	t.entry[n] = false
}

// childAt returns the index of the child of n specified by b.
func (t *tree[B]) childAt(n nodeRef, b bit) nodeRef {
	if b == bitR {
		return t.right[n]
	}
	return t.left[n]
...
func (t *tree[B]) childAtBool(n nodeRef, b bool) nodeRef {
	if b {
		return t.right[n]
	}
	return t.left[n]
...
func (t *tree[B]) setChildAt(b bit, n, o nodeRef) {
	if b == bitR {
		t.right[n] = o
		return
	}
...
func (t *tree[B]) setChild(n, o nodeRef) nodeRef {
	if o == absent {
		return o
	}
	//oKey := t.key[o]
...
// TODO document
func (t *tree[B]) Cursor() treeCursor[B] {
	return treeCursor[B]{t, absent}
}
...
type treeCursor[B KeyBits[B]] struct {
	*tree[B]
	node nodeRef
}

// tc2 is used for traversing two trees simultaneously.
type tc2[B KeyBits[B]] [2]treeCursor[B]

// Key returns the key of the current node.
func (t treeCursor[B]) Key() key[B] {
	return key[B]{t.len[t.node], t.offset[t.node], t.bits[t.node]}
}

func (t treeCursor[B]) HasEntry() bool {
	return t.entry[t.node]
}

func (t treeCursor[B]) ClearEntry() treeCursor[B] {
...
func (t treeCursor[B]) ChildAt(b bit) (treeCursor[B], bool) {
	child := t.tree.childAt(t.node, b)
	return treeCursor[B]{t.tree, child}, child != absent
}

// ChildAtBool returns a cursor positioned at the child (selected by b) of the
// current node. If the child doesn't exist, ChildAt returns (t, false).
func (t treeCursor[B]) ChildAtBool(b bool) (treeCursor[B], bool) {
...
func (t treeCursor[B]) NewChild(k key[B]) treeCursor[B] {
	child := t.newNode(k)
	t.SetChild(child)
	return treeCursor[B]{t.tree, child}
}

// NewChildAt creates a zero-valued node as the child (selected by b) of the
// current node.
func (t treeCursor[B]) NewChildAt(b bit) treeCursor[B] {
	var k key[B]
	child := t.newNode(k)
...
func (t treeCursor[B]) NewParent(k key[B]) treeCursor[B] {
	t.SetOffset(k.len)
	parent := t.newNode(k)
	t.tree.setChild(parent, t.node)
	return treeCursor[B]{t.tree, parent}
}

// SetNode replaces the node referred to by the cursor with the provided one.
//func (t treeCursor[B]) SetNode(n node) treeCursor[B] {
//	t.nodes[t.node] = n
//	return t
//}
...
func (t treeCursor[B]) DeleteNode() {
	//t.key[t.node] = key[B]{}
...
func (t treeCursor[B]) SetOffset(offset uint8) treeCursor[B] {
...
func (t treeCursor[B]) stringImpl(indent string, pre string, hideVal bool) string {
	var ret string
	nk := t.Key()
	if hideVal || !t.HasEntry() {
		ret = fmt.Sprintf("%s%s%s\n", indent, pre, nk.StringRel())
	} else {
		ret = fmt.Sprintf("%s%s%s: %v\n", indent, pre, nk.StringRel(), t.tree.entry[t.node])
	}
...
func (t treeCursor[B]) Size() (size int) {
	if t.HasEntry() {
		size += 1
	}
	if left, ok := t.ChildAt(bitL); ok {
...
func (t treeCursor[B]) Copy() treeCursor[B] {
	return newTree[B](len(t.bits)).Cursor().CopyFrom(t)
}

// CopyFrom copies o to t.
// TODO: is it ok if this overwrites? yes.
func (t treeCursor[B]) CopyFrom(o treeCursor[B]) treeCursor[B] {
	s := stack[tc2[B]]{}
	s.Push(tc2[B]{o, t})
	for !s.IsEmpty() {
		c := s.Pop()
		...
		for _, bit := range eachBit {
			if srcChild, srcOk := src.ChildAt(bit); srcOk {
				s.Push(tc2[B]{srcChild, dst.NewChildAt(bit)})
			}
		}
	}
	return t
...
func (t treeCursor[B]) Insert(k key[B]) treeCursor[B] {
	tKey := t.Key()
	...
	case com == tKey.len:
		...
		if !ok {
			child = t.NewChild(k.Rest(tKey.len)).SetValue()
		}
		child = child.Insert(k)
		t.SetChild(child.node)
		return t
	// Inserting at a prefix of tKey; create a new parent node with t as its
	// sole child
	case com == k.len:
		return t.NewParent(k).SetValue()
	...
	default:
		...
		parent.NewChild(k.Rest(com)).SetValue()
		return parent
	}
}
...
// current node, if any.
func (t treeCursor[B]) Remove(k key[B]) nodeRef {
	tKey := t.Key()
	switch {
	// Removing t itself
...
func (t treeCursor[B]) SubtractKey(k key[B]) nodeRef {
	tKey := t.Key()
...
func (t treeCursor[B]) SubtractTree(o treeCursor[B]) nodeRef {
...
func (t treeCursor[B]) IsEmpty() bool {
	//n := t.Node()
...
func (t treeCursor[B]) MergeTree(o treeCursor[B]) treeCursor[B] {
...
func (t treeCursor[B]) intersectTreeImpl(
	o treeCursor[B],
...
//
// TODO: same problem as subtractTree; only makes sense for PrefixSets.
func (t treeCursor[B]) IntersectTree(o treeCursor[B]) treeCursor[B] {
	return t.intersectTreeImpl(o, false, false)
}

// insertHole removes k and sets t, and all of its descendants, to v.
func (t treeCursor[B]) insertHole(k key[B]) nodeRef {
	tKey := t.Key()
	switch {
	...
	case t.Key().IsPrefixOf(k):
		...
		if !siblingOk {
			//*sibling = newTree[B](t.key.next((^bit) & 1)).setValue(v)
			t.NewChild(tKey.Next(inv(bit))).SetValue()
		}
		//*child = newTree[B](t.key.next(bit)).insertHole(k, v)
		t.SetChild(child.insertHole(k))

		// Create a new sibling to receive v if needed, then continue traversing
		//bit := k.bit(tKey.seg.len)
		//child, sibling := t.children(bit)
		//if *sibling == nil {
		//	*sibling = newNode[B](tKey.next(inv(bit))).setValue(v)
		//}
		//*child = newNode[B](tKey.next(bit)).insertHole(k, v)
		return t.node
...
func (t treeCursor[B]) walk(path key[B], fn func(treeCursor[B]) bool) {
...
func (t treeCursor[B]) pathNext(path key[B]) (treeCursor[B], bool) {
...
//func (t treeCursor[B]) Get(k key[B]) (val T, ok bool) {
...
func (t treeCursor[B]) Contains(k key[B]) (ret bool) {
	for t, pathOk := t.pathNext(k); pathOk; t, pathOk = t.pathNext(k) {
		tKey := t.Key()
		if !tKey.IsZero() {
...
func (t *tree[B]) equalPrefix(n nodeRef, k key[B]) bool {
...
// TODO strict
func (t treeCursor[B]) Encompasses(k key[B]) (ret bool) {
...
// encompasses the provided key.
// TODO strict
func (t treeCursor[B]) EncompassesStrict(k key[B]) (ret bool) {
	b128 := k.content.To128()
...
// TODO strict
//func (t treeCursor[B]) RootOf(k key[B], _ bool) (outKey key[B], val T, ok bool) {
//	t.walk(k, func(n treeCursor[B]) bool {
...
// If strict == true, the key itself is not considered.
//func (t treeCursor[B]) ParentOf(k key[B], strict bool) (outKey key[B], val T, ok bool) {
//	t.walk(k, func(n treeCursor[B]) bool {
//		if n.Key().IsPrefixOf(k) && n.HasEntry() {
//			outKey = n.Key()
//			val, ok = n.Value()
...
func (t treeCursor[B]) DescendantsOf(k key[B], strict bool) (ret treeCursor[B]) {
	t.walk(k, func(n treeCursor[B]) bool {
...
// the tree.
//func (t treeCursor[B]) AncestorsOf(k key[B], strict bool) (ret treeCursor[B]) {
//	ret = newTree[B](1, 1).Cursor()
//	t.walk(k, func(n treeCursor[B]) bool {
...
// at the same time.
//func (t treeCursor[B]) Filter(o treeCursor[B, bool]) {
//	remove := make([]key[B], 0)
//	var k key[B]
//	t.walk(k, func(n treeCursor[B]) bool {
...
//func (t treeCursor[B]) FilterCopy(o treeCursor[B, bool]) treeCursor[B] {
//	ret := newTree[B](1, 1).Cursor()
//	var k key[B]
//	t.walk(k, func(n treeCursor[B]) bool {
...
func (t treeCursor[B]) OverlapsKey(k key[B]) (ret bool) {
	t.walk(k, func(n treeCursor[B]) bool {
		if !n.HasEntry() {
			return false
		}
